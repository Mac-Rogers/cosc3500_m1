...
int main() {
    ...
    const int TOTAL_FRAMES = WARMUP_FRAMES + MAX_FRAMES;
    ...
    while (frameCount < TOTAL_FRAMES) {
        ...
        for(int x = 0; x < WIDTH; x++) {
            for(int y = 0; y < HEIGHT; y++) {
                if(grid[x][y] == OFF) {
                    // Inlined countOnNeighbors function
                    int count = 0;
        ...
        if (frameCount <= TOTAL_FRAMES && frameCount > WARMUP_FRAMES) {
            int dataFrameNumber = frameCount - WARMUP_FRAMES;
            frameTimes.push_back(frameTimeMs);
            
            // Generate statistics after 100 data frames
            if (dataFrameNumber == MAX_FRAMES) {
                double sum = 0;
                ...
                outFile << "# Frame timing statistics (after " << WARMUP_FRAMES << " warmup frames)\n";
                for (size_t i = 0; i < frameTimes.size(); i++) {
                    outFile << "Frame " << (i + 1) << ": " << frameTimes[i] << " ms\n";
                }
                outFile.close();
                
                std::cout << "\nCompleted " << WARMUP_FRAMES << " warmup frames + " << MAX_FRAMES << " data frames" << std::endl;
                std::cout << "Statistics saved to frame_timing_stats.txt" << std::endl;
